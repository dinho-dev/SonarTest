...
const DivDescriptionContainer = styled.div`
  ...
  padding-right: 20px;
`;
export default function ContenidoJs(props: any) {
  return (
    <DivContainerContenido>
      <TituloH2>{jsJsonData[props.contenidoPresente]?.titulo}</TituloH2>
      <DivDescriptionContainer>
        <p>{jsJsonData[props.contenidoPresente]?.descripcion}</p>
        <div>
          Ejemplos:
          <div>
            {jsJsonData[props.contenidoPresente]?.ejemplos.map(
